...
export class RelationshipDegreesService {
  ...
  async create(createRelationshipDegreeDto: CreateRelationshipDegreeDto) {
    ...
    const result = await this.supabaseService.insert<RelationshipDegree>(
      'relationship_degree',
      snakeCaseData,
    );
  ...
  async findAll(
    ...
    const { data, count } =
      await this.supabaseService.selectWithCount<RelationshipDegree>(
        'relationship_degree',
        columns,
        {},
  ...
  async findOne(id: number) {
    const result = await this.supabaseService.select<RelationshipDegree>(
      'relationship_degree',
      'id, description, active, created_by, updated_by, created_at, updated_at',
      { id },
    );
  ...
  async update(
    id: number,
    updateRelationshipDegreeDto: UpdateRelationshipDegreeDto,
  ) {
    const snakeCaseData = toSnakeCase(updateRelationshipDegreeDto);
    const result = await this.supabaseService.update<RelationshipDegree>(
      'relationship_degree',
      snakeCaseData,
      { id },
    );
